...
In this section crash severity classes are described. Crash Classes are gathered in 3 groups: critical, possible critical, not critical. Some class prototypes are taken from the open source library [gdb-exploitable](https://github.com/jfoote/exploitable.git).
...
### Possible critical (PROBABLY\_EXPLOITABLE)

Possible critical classes are needed some extra (often manual) analysis steps to determine if control flow hijeck is possible or not. List of classes:
...
### Not critical (NOT\_EXPLOITABLE)

Not critical classes are needed extra analysis manual analysis to determine if control flow hijeck is possible or not. Also it could be deny of service crash. Lists of classes:
